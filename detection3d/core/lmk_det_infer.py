...
# print('visuble gpu: {0}'.format(os.environ['CUDA_VISIBLE_DEVICES']))
...
from ..dataset.landmark_dataset import read_image_list
...
def check_input(input_path):
    ...
    if os.path.isfile(input_path):
        if input_path.endswith('.csv'):
            file_name_list, file_path_list, _, _ = read_image_list(
                input_path, 'test')
        else:
...
def detection_single_image(image, model):
    ...
    landmark_mask_preds = convert_tensor_to_image(
        landmark_mask_preds, sitk.sitkFloat32)

    detected_landmark = []
    for i in range(0, model['num_landmark_classes']):
        # exclude the background
        landmark_mask_pred = landmark_mask_preds[i + 1]
        ...
        prob_threshold = 0.5
        landmark_mask_binary = np.zeros_like(
            landmark_mask_prob, dtype=np.int16)
        ...
        landmark_mask_cc = pick_largest_connected_component(
            landmark_mask_cc, [1])

        # only keep probability of the largest connected component
        landmark_mask_cc = sitk.GetArrayFromImage(landmark_mask_cc)
        masked_landmark_mask_prob = np.multiply(
            landmark_mask_cc.astype(float), landmark_mask_prob)

        # compute the weighted mass center of the probability map
        masked_landmark_mask_prob = sitk.GetImageFromArray(
            masked_landmark_mask_prob)
        masked_landmark_mask_prob.CopyInformation(iso_image)
        voxel_coordinate = weighted_voxel_center(
            masked_landmark_mask_prob, prob_threshold, 1.0)

        landmark_name = landmark_name_list[landmark_label_reorder[i]]
        if voxel_coordinate is not None:
            world_coordinate = masked_landmark_mask_prob.TransformContinuousIndexToPhysicalPoint(
                voxel_coordinate)
            ...
            detected_landmark.append(
                [landmark_name, world_coordinate[0],
                    world_coordinate[1], world_coordinate[2]]
            )
        else:
            print(
                "world coordinate of landmark {0} is not detected.".format(i))
            detected_landmark.append(
                [landmark_name, 0.0, 0.0, 0.0])

    detected_landmark_df = pd.DataFrame(
        data=detected_landmark, columns=['name', 'x', 'y', 'z'])
...
def detection(input_path, model_folder, gpu_id, return_landmark_file, save_landmark_file, save_prob, output_folder):
    ...
    for i, file_path in enumerate(file_path_list):
        ...
        for j in range(0, model['num_landmark_classes']):
            # exclude the background
            landmark_mask_pred = landmark_mask_preds[j + 1]
            landmark_mask_pred.CopyInformation(iso_image)

            if save_prob:
                prob_path = os.path.join(
                    output_folder, '{}_{}.mha'.format(file_name_list[i], j))
            ...
            landmark_mask_cc = sitk.GetArrayFromImage(landmark_mask_cc)
            masked_landmark_mask_prob = np.multiply(
                landmark_mask_cc.astype(np.float), landmark_mask_prob)
            ...
            if voxel_coordinate is not None:
                ...
                print("world coordinate of volume {0} landmark {1} is not detected.".format(
                    file_name_list[i], j))
        ...
        if save_landmark_file:
            detected_landmark_save_path = os.path.join(
                output_folder, '{}.csv'.format(file_name_list[i]))
            detected_landmark_df.to_csv(
                detected_landmark_save_path, index=False)
