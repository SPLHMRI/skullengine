...
from .conv_gn_relu3 import ConvGnRelu3, BottConvGnRelu3
...
class ResidualBlock3(nn.Module):
    ...
    def __init__(self, channels, ksize, stride, padding, num_convs):
        ...
        for i in range(num_convs):
            if i != num_convs - 1:
                layers.append(ConvGnRelu3(channels, channels,
                              ksize, stride, padding, do_act=True))
            else:
                layers.append(ConvGnRelu3(channels, channels,
                              ksize, stride, padding, do_act=False))
...
class BottResidualBlock3(nn.Module):
    ...
    def __init__(self, channels, ksize, stride, padding, ratio, num_convs):
        ...
        for i in range(num_convs):
            if i != num_convs - 1:
                layers.append(BottConvGnRelu3(channels, channels,
                              ksize, stride, padding, ratio, do_act=True))
            else:
                layers.append(BottConvGnRelu3(channels, channels,
                              ksize, stride, padding, ratio, do_act=False))
