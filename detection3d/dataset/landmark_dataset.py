...
from ..utils.image_tools import select_random_voxels_in_multi_class_mask, \
    crop_image, convert_image_to_tensor, get_image_frame
from ..utils.landmark_utils import is_world_coordinate_valid, is_voxel_coordinate_valid
...
def read_landmark_coords(image_name_list, landmark_file_path, target_landmark_label):
    ...
    for idx, image_name in enumerate(image_name_list):
        ...
        assert len(label_dict[image_name]['name']) == len(
            target_landmark_label.keys())
...
class LandmarkDetectionDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.augmentation_translation = np.array(
            augmentation_translation, dtype=np.float32)
    ...
    def global_sample(self, image):
        ...
        im_size_mm = [image.GetSize()[idx] * image.GetSpacing()[idx]
                      for idx in range(3)]
        ...
        for i in range(3):
            if im_size_mm[i] > crop_size_mm[i]:
                sp[i] = origin[i] + \
                    np.random.uniform(0, im_size_mm[i] - crop_size_mm[i])
    ...
    def center_sample(self, image):
        ...
        center = np.array([(origin[idx] + end_point_world[idx]) /
                          2.0 for idx in range(3)], dtype=np.double)
        return center
    ...
    def select_samples_in_the_landmark_mask(self, landmark_mask, landmark_df):
        ...
        if len(valid_landmark_idx) > 0:
            for _ in range(self.num_pos_patches_per_image):
                ...
                landmark_voxel = list(
                    landmark_mask.TransformPhysicalPointToIndex(landmark_world))
                if is_voxel_coordinate_valid(landmark_voxel, image_size):
                    perturbs_idx = np.random.randint(
                        0, len(self.positive_perturbs))
                    for id in range(3):
                        landmark_voxel[id] += self.positive_perturbs[perturbs_idx][id]
                        landmark_voxel[id] = max(
                            0, min(landmark_voxel[id], image_size[id] - 1))
        ...
        for voxel in sample_voxels:
            ...
            voxel_ep_x = min(image_size[0] - 1,
                             voxel_sp_x + self.sampling_size[0])
            voxel_sp_y = max(0, voxel[1] - self.sampling_size[1] // 2)
            voxel_ep_y = min(image_size[1] - 1,
                             voxel_sp_y + self.sampling_size[1])
            voxel_sp_z = max(0, voxel[2] - self.sampling_size[2] // 2)
            voxel_ep_z = min(image_size[2] - 1,
                             voxel_sp_z + self.sampling_size[2])
            selected_mask_npy[voxel_sp_z:voxel_ep_z, voxel_sp_y:voxel_ep_y, voxel_sp_x:voxel_ep_x] = \
                mask_npy[voxel_sp_z:voxel_ep_z,
                         voxel_sp_y:voxel_ep_y, voxel_sp_x:voxel_ep_x]
        ...
        for idx in range(len(landmark_df['label'])):
            ...
            reordered_selected_mask_npy[abs(
                selected_mask_npy - label) < 1e-1] = idx + 1

        reordered_selected_mask = sitk.GetImageFromArray(
            reordered_selected_mask_npy)
    ...
    def __getitem__(self, index):
        ...
        if self.sampling_method == 'GLOBAL':
            ...
            raise ValueError(
                'Only support CENTER, GLOBAL, MASK, and HYBRID sampling methods')

        # random translation
        center += np.random.uniform(-self.augmentation_translation,
                                    self.augmentation_translation, size=[3])
        ...
        landmark_mask = crop_image(
            landmark_mask, center, self.crop_size, self.crop_spacing, 'NN')
        ...
        landmark_coords_tensor = torch.from_numpy(
            np.array(landmark_coords_list, dtype=np.float32))
