...
from ..utils.image_tools import normalize_image, get_mean_std_from_image
    ...
    def __call__(self, image):
        ...
        if isinstance(image, sitk.Image):
            ...
            for idx, im in enumerate(image):
                ...
                image[idx] = normalize_image(
                    im, self.mean, self.stddev, self.clip)
            return image

        else:
            raise ValueError(
                'Unknown type of input. Normalizer only supports Image3d or Image3d list/tuple')

    def to_dict(self):
        """ convert parameters to dictionary """
        obj = {'type': 0, 'mean': self.mean,
               'stddev': self.stddev, 'clip': self.clip}
        return obj
...
class AdaptiveNormalizer(object):
    ...
    def normalize(self, single_image):
        ...
        assert isinstance(
            single_image, sitk.Image), 'image must be an image3d object'

        normalize_mean, normalize_stddev = get_mean_std_from_image(
            single_image)
