...
def weighted_voxel_center(image, threshold_min, threshold_max):
    ...
    weighted_center_x = np.sum(np.multiply(
        image_npy, image_npy_x)) / weight_sum
    weighted_center_y = np.sum(np.multiply(
        image_npy, image_npy_y)) / weight_sum
    weighted_center_z = np.sum(np.multiply(
        image_npy, image_npy_z)) / weight_sum
    ...
    for i in idxs:
        ...
        if crops is not None:
            ...
            for modality_idx, image in enumerate(images):
                ...
                sitk.WriteImage(image, os.path.join(
                    case_out_folder, 'batch_{}_crop_{}.nii.gz'.format(i, modality_idx)))
        ...
        if masks is not None:
            ...
            sitk.WriteImage(mask, os.path.join(
                case_out_folder, 'batch_{}_mask.nii.gz'.format(i)))

        if outputs is not None:
            cls_num = outputs.size()[1]
            for cls in range(cls_num):
                output = convert_tensor_to_image(
                    outputs[i, cls].data, dtype=np.float32)
                set_image_frame(output, frames[i].numpy())
                sitk.WriteImage(output, os.path.join(
                    case_out_folder, 'batch_{}_output_{}.nii.gz'.format(i, cls)))
...
def copy_image(source_image, target_start_voxel, target_end_voxel, target_image):
    ...
    target_start_world = target_image.TransformIndexToPhysicalPoint(
        target_start_voxel)
    source_start_voxel = source_image.TransformPhysicalPointToIndex(
        target_start_world)
    paste_size = [int(target_end_voxel[idx] - target_start_voxel[idx])
                  for idx in range(3)]
...
def image_partition_by_fixed_size(image, bbox_start_voxel, bbox_end_voxel,
    ...
    """
    image_size, image_spacing, image_origin = image.GetSize(
    ), image.GetSpacing(), image.GetOrigin()
    for idx in range(3):
        assert image_size[idx] >= max_stride and image_size[idx] % max_stride == 0

    bbox_size = [min(image_size[idx], bbox_end_voxel[idx] -
                     bbox_start_voxel[idx]) for idx in range(3)]
    for idx in range(3):
    ...
    box_size = [int(partition_size[idx] / image_spacing[idx] + 0.5)
                for idx in range(3)]
    for idx in range(3):
    ...
    stride_size = [int(partition_stride[idx] / image_spacing[idx] + 0.5)
                   for idx in range(3)]
    for idx in range(3):
        stride_size[idx] = min(bbox_size[idx], stride_size[idx])

    num_partitions = [int(np.ceil(
        (bbox_size[idx] - box_size[idx]) / stride_size[idx]) + 1) for idx in range(3)]
    ...
    for idx in range(0, num_partitions[0]):
        for idy in range(0, num_partitions[1]):
            for idz in range(0, num_partitions[2]):
                ...
                end_voxel = [start_voxel[0] + box_size[0],
                             start_voxel[1] + box_size[1], start_voxel[2] + box_size[2]]
                for dim in range(3):
...
def convert_tensor_to_image(tensor, dtype):
    ...
    if tensor.dim() == 3:
        ...
        raise ValueError(
            'Only supports 3-dimsional or 4-dimensional image volume')
...
def resample_spacing(image, resampled_spacing, max_stride, interp_method):
    ...
    out_size = [int(in_size[idx] * in_spacing[idx] /
                    out_spacing[idx] + 0.5) for idx in range(3)]
    for idx in range(3):
...
def pick_largest_connected_component(mask, labels):
    ...
    for idx in range(1, len(labels)):
        largest_cc_multi = sitk.Add(
            largest_cc_multi, labels[idx] * largest_cc_binaries[idx])
...
def get_bounding_box(mask, selected_labels):
    ...
    try:
        ...
        bbox_end_voxel = [bbox_start_voxel[0] + bbox[3],
                          bbox_start_voxel[1] + bbox[4], bbox_start_voxel[2] + bbox[5]]
    except:
