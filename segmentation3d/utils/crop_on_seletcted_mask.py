...
def get_anchor(lm_path):
  if lm_path.endswith('.csv'):
    ...
    landmark_all = pd.read_excel(lm_path)
...
def crop_patch(image_path, patch_path, cropping_center):
  ...
  org_path = os.path.join(image_path,'img.mha') #'/home/admin/test/data_v1/case_17_cbct_patient/org.mha'
  seg_path = os.path.join(image_path,'merged_seg.mha') #''/home/admin/test/data_v1/case_17_cbct_patient/seg.mha'
  ...
  cropped_image_path = os.path.join(patch_path,'cropped_org.mha') #'/home/admin/test/test_crop/cropped_org.mha'
  cropped_seg_path = os.path.join(patch_path, 'cropped_seg.mha') #'/home/admin/test/test_crop/cropped_seg.mha'
...
def main(input_path, saving_path):
  if not os.path.isdir(saving_path):
    os.mkdir(saving_path)

  landmark_path = os.path.join(input_path, 'data_ldmk')
  image_path = os.path.join(input_path, 'data')
  case_list = os.listdir(landmark_path)
  # case_list = os.path.splitext(case_list)[0]
  for case in case_list:
    print('Current File:{}'.format(case))
    case_path = os.path.join(landmark_path,case)
    if os.path.isdir(case_path):
      continue
    lm_path = os.path.join(landmark_path, case)
    case_name = os.path.splitext(case)[0]
    img_path = os.path.join(image_path, case_name)
    patch_path = os.path.join(saving_path, case_name)
...
if __name__ == '__main__':
  ...
  main('/data/deng/New_GT/', '/data/deng/New_GT/crop_ION_R_pre_p')
